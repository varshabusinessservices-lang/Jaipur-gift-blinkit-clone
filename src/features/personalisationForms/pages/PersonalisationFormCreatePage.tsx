...
import {
  ArrowLeft, Plus, Trash2, Copy, Save, AlertCircle, ArrowUp, ArrowDown, Settings, Check, HelpCircle
} from 'lucide-react';
...
export function PersonalisationFormCreatePage() {
    ...
    try {
      ...
      const payload = {
        ...
        fields: fields.map((f, idx) => ({
          ...f,
          id: f.id.startsWith('temp-') ? undefined : f.id, // backend will assign uuid
          sortOrder: idx + 1,
        })),
      };

      await personalisationFormsService.createForm(payload);
      toast.success('Personalisation Form created successfully');
      navigate('/admin/personalisation-forms');
    } catch (error) {
      console.error(error);
      toast.error('Failed to create personalisation form');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mt-0.5">Build New Form</h1>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Custom Slug (Auto Generated)</label>
              <input
                type="text"
                value={slug}
